...
class ExportarActividadesController extends Controller
{
    public function export(Request $request)
    {
        ...
        // Incluir eliminados si se indicó
        if (!empty($data['incluir_eliminados'])) {
            $query->withTrashed();
        }

        // Filtro por actividades propias
        if (!empty($data['propio']) && $data['propio'] === 'true') {
            $query->where('user_id', auth()->id());
        } else {
            // Filtro por usuario específico si no es reporte propio
            if (!empty($data['usuario'])) {
                $ids = is_array($data['usuario']) ? $data['usuario'] : [$data['usuario']];
                $query->whereIn('user_id', $ids);
            }

            // Aquí podrías agregar más filtros si deseas respetar unidad, gerencia, etc.
        }

        // Filtro por fechas según modo
        if (($data['modo_fecha'] ?? null) === 'quincena' && !empty($data['quincena_seleccionada']) && !empty($data['year'])) {
        ...
        } elseif (($data['modo_fecha'] ?? null) === 'mes' && !empty($data['mes_seleccionado']) && !empty($data['year'])) {
            $inicio = "{$data['year']}-{$data['mes_seleccionado']}-01";
            $fin = date("Y-m-t", strtotime($inicio));
            $query->whereBetween('fecha', [$inicio, $fin]);
        } elseif (($data['modo_fecha'] ?? null) === 'anual') {
            $inicio = "{$data['year']}-01-01";
            $fin = "{$data['year']}-12-31";
            $query->whereBetween('fecha', [$inicio, $fin]);
        } elseif (($data['modo_fecha'] ?? null) === 'personalizado') {
        ...
        $tituloModo = match ($data['modo_fecha'] ?? null) {
            'anual' => 'Anual',
            'quincena' => 'Quincenal',
            'mes' => 'Mensual',
        ...
        if (!empty($data['unidad_administrativa']) && empty($data['propio'])) {
        ...
        $mesNombre = null;
        $quincenaTexto = null;
        if (($data['modo_fecha'] ?? null) === 'quincena' && !empty($data['quincena_seleccionada'])) {
        ...
        $rangoFechas = match ($data['modo_fecha'] ?? null) {
            'anual' => $data['year'],
